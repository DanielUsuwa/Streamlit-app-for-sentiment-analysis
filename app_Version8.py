...
# -*- coding: utf-8 -*-
"""
Streamlit Sentiment Analyzer — improved desktop layout, compact mobile, multiple text columns support.

Changes in this version:
- Removed the extra intro sentence for a cleaner header.
- Wide layout improves desktop UX; controls remain reachable on mobile.
- Added support for up to three text columns (Text column 1 is required, Text column 2 & 3 optional).
  Selected text columns are concatenated into a single combined text used for analysis.
- Removed the two help lines from the Help & notes expander as requested.
- Safe WordCloud fallback remains available when the wordcloud package isn't installed.
- Fixed previous Altair schema issues by using compatible color schemes and defensive rendering.
"""
...
# -------------------------
# Controls (top row) - compact and arranged to work on both mobile & desktop
# -------------------------
...
with controls:
    ...
    with st.expander("Advanced (hidden)", expanded=False):
        ...
        st.markdown("Compact help available in 'Help & notes' at the bottom.")
...
# Minimal Help (collapsed) - removed the two requested lines
