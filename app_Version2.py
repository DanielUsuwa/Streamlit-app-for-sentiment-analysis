...
# -*- coding: utf-8 -*-
"""
Streamlit app for simple sentiment analysis (VADER) with preprocessing and word cloud.

Features:
- Upload a CSV (or use sample data)
- Choose text column
- Preprocess (HTML/URLs removal, lowercasing, punctuation/number removal, stopword removal, POS-aware lemmatization)
- Score with VADER and show neg/neu/pos/compound + simple labels
- Visual sentiment distribution, word cloud for positive reviews
- Download results (CSV) and wordcloud image
...
def ensure_nltk_resources():
    # Downloads are idempotent; wrap in try/except so we show friendly errors
    resources = [
        "punkt",
        "stopwords",
        "wordnet",
    ...
    for res in resources:
        try:
            ...
            with st.spinner(f"Downloading NLTK resource: {res} ..."):
...
st.markdown(
    ...
    "cleaned text, a sentiment distribution plot, and a word cloud for positive reviews."
)
...
with st.sidebar:
    ...
    st.info(
        "If the app stalls during first run, it's likely downloading NLTK resources (vader, punkt, etc.).\n"
        "You can run `python -c \"import nltk; nltk.download('vader_lexicon')\"` locally first or be patient; "
        "the app will display a spinner when downloading."
    )
...
fig = px.pie(counts, names="label", values="count", title="Sentiment distribution", hole=0.35, color="label")
