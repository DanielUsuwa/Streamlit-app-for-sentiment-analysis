...
# -*- coding: utf-8 -*-
"""
Streamlit app (version 2) for sentiment analysis (VADER) with preprocessing and word cloud.
This variant uses Altair for charts (no plotly dependency) so it runs smoothly on Streamlit Cloud
...
# Visualization (use Altair instead of Plotly)
...
def ensure_nltk_resources():
    ...
    for name, path in resources.items():
        try:
            ...
            # show spinner so UI doesn't appear stalled
...
st.markdown(
    ...
    "cleaned text, a sentiment distribution chart, and a word cloud for positive reviews."
)
...
with st.sidebar:
    ...
    st.info(
        ...
        "You can pre-download them locally with `python -c \"import nltk; nltk.download('vader_lexicon')\"`."
    )
...
if len(counts) == 0:
    ...
    # Use an arc mark for a pie/donut
